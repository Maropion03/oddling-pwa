      ...
      <div className="page-wrap album-page">
            ...
            <ol className="mutation-history">
              {[...state.mutations].reverse().map((mutation, index) => (
                <li key={mutation.id}>
                  ...
                  <div><time>{compactDate(mutation.date)}</time><strong>{plainText(mutation.label)}</strong><p>{mutation.previousToken ? "替换了之前的同槽位部件" : "永久留在了角色身上"}</p></div>
        ...
        {tab === "relations" && (
          <div className="sticker-grid">
            ...
            {!relationshipStickers.length && <div className="empty-state"><p>还没有人和它发生关系事件<br/>分享一次就可能掉落第一张</p></div>}
          </div>
        )}
      </div>
    </AppShell>
